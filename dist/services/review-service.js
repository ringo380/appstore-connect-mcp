export class ReviewService {
     ...
     */
    async getCustomerReviews(appId, limit = 100) {
        try {
            ...
            if (response2.data)
                return this.formatCustomerReviews(response2.data);
            return [];
        }
    }
    /**
     ...
     */
    async getReviewResponses(appId) {
        try {
     ...
     */
    async getReviewSentiment(appId) {
    ...
    async getReviewSummary(appId) {
        ...
        return {
            ...
            topIssues: sentiment.topTopics.filter((t) => ['bug', 'crash', 'problem'].includes(t.topic)),
            highlights: {
                mostLovedFeatures: sentiment.topTopics.filter((t) => ['feature', 'design', 'ui'].includes(t.topic)),
     ...
     */
    generateRecommendations(metrics, sentiment) {
