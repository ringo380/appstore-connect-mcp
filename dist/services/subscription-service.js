...
export class SubscriptionService {
    ...
    async getSubscriptionRenewals(date) {
    ...
    async getMonthlySubscriptionAnalytics(year, month) {
        const analytics = {
            year,
            month,
        ...
        for (let i = 0; i < dates.length; i += BATCH_SIZE) {
            ...
            const batchResults = await Promise.allSettled(batch.map(dateStr => this.getSubscriptionRenewals(dateStr)));
            ...
            if (data.salesData) {
                ...
                if (data.salesData.subscriptionTypes) {
                    for (const [type, count] of Object.entries(data.salesData.subscriptionTypes)) {
                        analytics.subscriptionTypes.set(type, (analytics.subscriptionTypes.get(type) || 0) + count);
                    }
                }
                if (data.salesData.countries) {
                    for (const [country, revenue] of Object.entries(data.salesData.countries)) {
                        analytics.geographicDistribution.set(country, (analytics.geographicDistribution.get(country) || 0) + revenue);
                    }
                }
            }
    ...
    analyzeSubscriptionData(parsedData) {
        const analysis = {
            totalSubscribers: 0,
            activeSubscribers: 0,
            newSubscribers: 0,
            ...
            revenue: 0,
            averagePrice: 0,
            subscriptionsByProduct: new Map(),
            subscriptionsByCountry: new Map()
        };
    ...
    extractSubscriptionFromSales(parsedData) {
        const extracted = {
            subscriptionRevenue: 0,
            oneTimeRevenue: 0,
            newSubscriptions: 0,
        ...
        for (const row of parsedData.rows) {
            ...
            const country = row['Country Code'] || 'Unknown';
            const sku = row['SKU'] || '';
            const isSubscription = productType.includes('Auto-Renewable') ||
