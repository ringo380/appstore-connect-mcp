export class AnalyticsService {
    ...
    async getAppAnalytics(request) {
        ...
        const targetReport = reports.find((r) => keywords.some(kw => r.attributes?.name?.toLowerCase().includes(kw))) || reports[0];
        ...
        if (!reportId) {
            return {
                ...
                availableReports: reports.map((r) => r.attributes?.name),
    ...
    async pollForReports(requestId, timeoutMs) {
